...
class Polynomial :
    ...
    def __add__(self, rhsPoly):
        ...
        # Or if rhs contains more terms append them.c
     ...
     # Polynomial subtraction: newPoly = self - rhsPoly.
    def __sub__(self, rhsPoly):
        assert self.degree() >= 0 and rhsPoly.degree() >= 0, "Subtraction only allowed on non -empty polynomials."
        newPoly = Polynomial()
        nodeA = self._polyHead
        nodeB = rhsPoly._polyHead
    ...
    def __mul__(self, rhsPoly):
        ...
        while nodeA is not None:
            while nodeB is not None:
                deg = nodeA.degree + nodeB.degree
                koef = nodeA.coefficient * nodeB.coefficient
                if deg in koefs:
                    koefs[deg] += koef
                else:
                    koefs[deg] = koef
        ...
        newPoly = Polynomial()
        for deg in reversed(list(koefs.keys())):
            newPoly._appendTerm(deg, koefs[deg])
    ...
    def __str__(self):
        ...
        while node is not None:
            res += str(node.coefficient) + '*x^' + str(node.degree) + '+'
            node = node.next
        return res[:-1]
